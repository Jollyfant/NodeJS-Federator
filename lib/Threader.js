...
Threader.prototype.OpenThread = function(thread) {
  ...
  // Set a timeout on each request (status code: 504)
  var threadRequestTimeout = setTimeout(function() {
    thread.statusCode = 504;
...
Threader.prototype.FlushThread = function(thread) {
  ...
  if(thread.dataBuffer.length) {
    ...
    if(!thread.running) {
    ...
    } else if(this.Handler.stream.service === "dataselect") {
      ...
      var sliceIndex = dataBuffer.length - (dataBuffer.length % MAXIMUM_RECORD_LENGTH_BYTES);
      ...
      thread.nBytesBuffered = dataBuffer.length % MAXIMUM_RECORD_LENGTH_BYTES;
